...
export default function RepositoryCard({ repository }: { repository: GithubRepository }) {
  return (
    <Link href={`/repositories/${repository.id}`} className="panel group block p-6 hover:-translate-y-1 hover:border-teal-300">
      <div className="flex items-center justify-between gap-3">
        ...
        <Badge value="success">Connected</Badge>
      </div>
      <h3 className="mt-5 break-all text-lg font-black text-slate-850">{repository.owner}/{repository.repo}</h3>
      <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
        <span className="rounded-full bg-slate-100 px-3 py-1">branch: {repository.default_branch || "-"}</span>
        <span className="rounded-full bg-slate-100 px-3 py-1">ID: {repository.id}</span>
      </div>
